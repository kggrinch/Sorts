...
int main()
{
	...
	//IterativeMergeSort big
	points = 3;
	int array_size = 1563;
	p_arr = new vector<int>;
	for (int i = 0; i < array_size; i++)
	{
		(*p_arr).push_back(rand());
	}
	p_correct_arr = new vector<int>(*p_arr);
	LocalSort(*p_correct_arr, 0, (*p_correct_arr).size() - 1);
	IterativeMergeSort(*p_arr, 0, (*p_arr).size() - 1);
	...
	{
		miss += points;
		cout << "IterativeMergeSort Big Failed: -" << points << endl;
		//DisplayVector(*p_arr);
		//DisplayVector(sorted_array);
	}
	delete p_arr;
